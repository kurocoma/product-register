...
- **FR1 競合検出**: 対象商品ごとに、SKUのJANコード（13桁）をキーワードに楽天市場を価格昇順で検索し、
  ...
  - JANで有効な競合が見つからない場合は掲載商品名で1回だけ再検索し、商品名の一致度で検証する。
  - 同一商品判定のガード: 自店販売価格の 0.5〜2.0 倍の価格帯のみ有効（入数違い・別商品の混入を防ぐ）。
- **FR2 倍率決定**: 最安値上位N店（既定3店）の最大ポイント倍率 +1倍。上限（既定3倍）で打ち止め（capped として記録）。
  競合に勝てる倍率が1倍以下なら変倍しない。
- **FR3 自動反映**: 決定した倍率を RMS Item API 2.0 の `pointCampaign`（倍率＋適用期間、既定7日間）で自店商品へ PATCH する。
  - 既に目標倍率で適用中かつ残り期間が36時間超なら何もしない（unchanged）。
  - 競合が弱くなり変倍不要になったら解除を試みる（clear。API が解除を受け付けない場合は期間満了での自然失効に委ね、その旨を記録）。
- **FR4 定期実行**: 1日2回（既定 9:00 / 21:00、タスク登録batで変更可）。アプリ（next dev）の起動有無に依存しない単体スクリプト。
- **FR5 手動実行**: 画面から dry-run（照会のみ）と本実行をいつでも実行できる（1回の上限50件）。
...
- 1商品の失敗で全体を止めない（件別 try/catch、結果に error 記録）。
- HTTP ルートは `maxDuration=300` と件数上限50で Next.js のタイムアウト内に収める。全件処理はスケジューラ用スクリプトで行う。
...
- **R1 `pointCampaign` フィールド**: RMS Item API 2.0 の item リソースに `pointCampaign`（applicablePeriod/benefits.pointRate）が
  ...
  **初回 dry-run 後の実機 PATCH 1件で要確認**。フィールド名が異なる場合も `point-campaign.ts` の定数修正のみで済む構造にした。
  解除（`pointCampaign: null`）が拒否された場合は自然失効に委ねる設計。
