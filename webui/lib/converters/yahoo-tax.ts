 ...
 *   （四捨五入誤差は |0.5| 以下 → ÷(1+r/100) 後の誤差は 0.5/(1+r/100) < 0.5 のため必ず元に戻る）
 * 丸めは既存 YahooConverter と同一の四捨五入（Math.floor(x + 0.5)）に固定する。 */

/** 税抜 → 税込（Yahoo へ送る price / original_price 用）。 */
export function yahooTaxInclusive(priceExclusive: number, taxRate: number): number {
  return Math.floor(priceExclusive * (1 + taxRate / 100) + 0.5);
}

/** 税込 → 税抜（Yahoo getItem の Price / OriginalPrice の取込用）。 */
export function yahooTaxExclusive(priceInclusive: number, taxRate: number): number {
  return Math.floor(priceInclusive / (1 + taxRate / 100) + 0.5);
}
